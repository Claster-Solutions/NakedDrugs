...
export default function BlogCard(p: Props) {
    ...
    return (
        <Link
            href={`/blog/${p.blog.id}`}
            className="m-4 flex max-h-56 min-h-40 flex-row justify-between rounded-xl bg-white shadow-lg hover:shadow-xl"
        >
            <div className="w-4/12 ">
                <img className="h-full w-auto object-contain" src={image.src}></img>
            </div>
            <div className="flex w-full flex-col items-start">
                <h2 className="text-xl font-bold">{p.blog.title}</h2>
